...
def recommend_courses(user_mbti, user_ceec, course_df, top_n=10):
    """
    根據用戶 MBTI 前四個字母和 CEEC 推薦課程。
    """
    # 計算匹配分數
    updated_df = calculate_matching_score(user_mbti, user_ceec, course_df)
    ...
    recommended_courses = updated_df[['id', '課程名稱', 'matching_score', 'MBTI_type', 'CEEC_type', 'link', 'category']].sort_values(
    by='matching_score', ascending=False
    ).head(top_n)
    
    recommended_courses = recommended_courses.where(pd.notnull(recommended_courses), None)
    # 格式化輸出
